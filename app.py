...
JOB_DESC_FILE = "job_description.json"
    ...
    if request.method == "POST":
        try:
            ...
            if not job_description_keywords:
                return jsonify({"error": "No job description set. Please set a job description first."}), 400
            ...
            for filename in data["selected_resumes"]:
                try:
                    ...
                    if not text:
                        results.append({"filename": filename, "error": "Failed to extract text"})
                        continue
                        
                    resume_keywords = extract_keywords(text)
                    score, missing_requirements = calculate_match_score(resume_keywords, job_description_keywords)
                    results.append({
                        "filename": filename,
                        "score": score,
            ...
            return jsonify({"error": "Internal server error. Please check the logs for more details."}), 500
    else:
        try:
...
def upload_resume():
    try:
        ...
        return jsonify({"message": f"Uploaded {len(results)} files successfully!", "results": results}), 200
    except Exception as e:
